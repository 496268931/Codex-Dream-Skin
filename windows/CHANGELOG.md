...
- Codex 主区顶部可显示当前中转 provider 的实时余额：启动及 provider 切换时立即查询，之后每 30 秒刷新；查询只在本机 Node 守护进程中进行，Token 不进入主题、日志或渲染页面。OpenAI 官方登录会隐藏余额，不兼容的 provider 会明确显示不支持。
- 新增 Windows 内置主题「异色超级裂空座」：以宇宙中的画龙点睛场景作为 2560 × 1440 深色壁纸，可从托盘「已保存主题」直接切换。
