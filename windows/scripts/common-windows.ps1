...
function Install-DreamSkinRuntimeEngine {
  ...
  # Every bundled preset the source ships must be complete. Enumerate the
  # source tree (mirroring theme-windows.ps1 seeding) so a release-shaped
  # source carrying only the public presets stays valid while any personal
  # preset present must still be fully staged.
  $presetSourceRoot = Join-Path $sourceRoot 'presets'
  if (Test-Path -LiteralPath $presetSourceRoot -PathType Container) {
    foreach ($presetSource in Get-ChildItem -LiteralPath $presetSourceRoot -Directory -Filter 'preset-*') {
      $required += @(
        "presets\$($presetSource.Name)\theme.json",
        "presets\$($presetSource.Name)\background.jpg"
      )
    }
  }
