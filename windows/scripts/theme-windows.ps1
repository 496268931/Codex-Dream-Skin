...
function Initialize-DreamSkinThemeStore {
  ...
  # Seed and refresh every bundled preset, including user-maintained presets.
  ...
  if (Test-Path -LiteralPath $presetRoot -PathType Container) {
    foreach ($sourceDirectory in Get-ChildItem -LiteralPath $presetRoot -Directory -Filter 'preset-*') {
      ...
      $imageName = [System.IO.Path]::GetFileName("$($source.Theme.image)")
      if ($presetId -cne $sourceDirectory.Name -or $imageName -ne "$($source.Theme.image)") {
        throw "Bundled preset metadata is invalid: $($sourceDirectory.FullName)"
      }
  ...
  if (Test-Path -LiteralPath $activeTheme -PathType Leaf) {
    ...
    if ($activeId -ceq $bundledPresetId) { $refreshSource = $assetRoot }
      ...
      $candidate = Join-Path $SkillRoot ("presets\{0}" -f $activeId)
      if (Test-Path -LiteralPath (Join-Path $candidate 'theme.json') -PathType Leaf) { $refreshSource = $candidate }
    }
