...
((cssText, artDataUrl, themeConfig) => {
  ...
  const BALANCE_ID = "codex-dream-skin-balance";
  ...
  const renderBalance = (state = window[BALANCE_KEY]) => {
    if (!balanceNode) return;
    const status = ["hidden", "loading", "ok", "stale", "error", "unsupported"].includes(state?.status)
      ? state.status : "hidden";
    balanceNode.dataset.status = status;
    balanceNode.hidden = status === "hidden" || status === "unsupported";
    const provider = balanceNode.querySelector("[data-balance-provider]");
    const value = balanceNode.querySelector("[data-balance-value]");
    if (provider) provider.textContent = String(state?.provider || "中转 API").slice(0, 80);
    if (value) value.textContent = status === "loading" ? "查询中..."
      : status === "ok" || status === "stale"
        ? `${Number(state.remaining).toFixed(2)} ${String(state.unit || "USD")}`
        : status === "error" ? "余额查询失败" : "";
  };
  const ensureBalance = () => {
    if (!document.body) return;
    if (!balanceNode) {
      balanceNode = document.createElement("div");
      if (!balanceNode || typeof balanceNode.appendChild !== "function" ||
        typeof balanceNode.querySelector !== "function") {
        balanceNode = null;
        return;
      }
      balanceNode.id = BALANCE_ID;
      balanceNode.dataset.dsPart = "provider-balance";
      balanceNode.innerHTML = '<span data-balance-provider></span><strong data-balance-value></strong>';
      document.body.appendChild(balanceNode);
      balanceHandler = (event) => renderBalance(event?.detail);
      window.addEventListener(BALANCE_EVENT, balanceHandler);
    }
    renderBalance();
  };
  ...
  const cleanup = () => {
    ...
    removeParts();
    if (balanceHandler) window.removeEventListener(BALANCE_EVENT, balanceHandler);
    balanceNode?.remove();
    balanceNode = null;
    balanceHandler = null;
