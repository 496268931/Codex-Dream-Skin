...
- AI 只在用户明确授权的仓库目录内执行 Git 操作；未授权时仅做只读检查。
- 用户要求准备提交时，AI 必须提供中文 commit 信息和命令，由用户确认后完成提交与推送。
- 默认从官方 remote `origin` 更新，个人 remote `wy` 仅用于推送；不得把个人仓库作为官方更新来源。
- 不提交密钥、令牌、个人路径或本机状态文件。完整流程见 `docs/GIT_WORKFLOW.md`。
