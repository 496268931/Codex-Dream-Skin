...
## Personal Windows update to upstream v1.5.12 (2026-08-08)

- [scope] Merge upstream `origin/main` v1.5.12 (`6f789be`) into personal
  `main` (`1052496`) while preserving the Windows relay-provider balance UI,
  the bundled Shiny Mega Rayquaza preset, the local-only Git workflow, and the
  stricter user-owned Git-operation rules.
- [diagnosed] The installed July 21 engine cannot verify Codex
  `26.730.8199.0`: `verify.log` reports `No page matched the expected Codex
  shell markers`. Skin and balance disappear together because both depend on
  the same renderer injection session; theme files remain intact.
- [merge state] User fetched upstream and started `git merge origin/main`.
  Ten files are conflicted. A safety branch
  `backup/wy-before-official-update-20260807` exists. The agent must edit only
  ordinary files and must not stage, commit, finish/abort the merge, or push.
- [strategy] Keep upstream v1.5.12 selectors, readiness checks, Safe CSS,
  theme schema, and shared `runtime/` assets as the base. Reapply the balance
  presentation in shared renderer/CSS sources and keep the provider query in
  Windows-only Node code. Upgrade the Rayquaza preset to the current bundled
  theme contract without overwriting user-created themes.
- [done] Resolved the conflicts against the upstream v1.5.12 baseline, then
  re-applied the balance presentation in the shared renderer/CSS sources while
  keeping the provider query in Windows-only Node code. The runtime preset
  completeness check now enumerates the source tree (so a release-shaped source
  that ships only the public Gothic preset stays valid), and
  `provider-balance.mjs` was added to the release fixture, the installer
  payload manifest, and the same-version repair manifest. The full Windows
